...
3. Ask the user for the stock ticker (e.g., "600519") they want to analyze. If they provide a company name instead, use google search to find the corresponding ticker which should be a 6-digit string.

4. Throughout the process, explicitly state which subagent you are calling, request the required inputs, and after each call, acknowledge receipt of the full output.

5. Maintain consistent state keys: fundamental_output, technical_output, fund_output, policy_output.
