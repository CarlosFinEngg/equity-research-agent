...
def calculate_technical_indicators(symbol: str) -> dict:
    ...
    where "today" is determined in the China (Asia/Shanghai) timezone, then calculate the following:
      ...
      7. Technical events with refined Chinese text descriptions:
         ...
         - 持续缩量天数: 累计连续缩量天数
         - 向上突破:
             * 5日均线, 10日均线, 20日均线, 30日均线, 60日均线, 90日均线, 250日均线
             * 布林带上轨
         - 向下突破:
             * 5日均线, 10日均线, 20日均线, 30日均线, 60日均线, 90日均线, 250日均线
             * 布林带下轨
    ...
    Returns (dict):
      ...
      error_message (str, optional): When status="error", contains an explanation
      indicators (list[dict], optional): When status="success", a list of records for each trading day,
        including all original fields, computed indicators, and signal descriptions. Example record format:
        {
          "日期": "2022-01-03",
          "开盘": 12.34,
          "收盘": 12.56,
    ...
    try:
        ...
        window_vol = 20
        df["volatility"] = df["log_return"].rolling(window=window_vol).std() * np.sqrt(252)
        ...
        df["volume_amplification"] = df["成交量"] / df["vol_ma20"]
        ...
        df["MA90"] = df["收盘"].rolling(window=90).mean()
        ...
        # # 9.4 向上突破 / 向下突破: 赋值对应均线名称列表与布林带标志
        # df["突破均线"] = df.apply(lambda row: [ma for ma in ["5日均线","10日均线","20日均线","30日均线","60日均线","90日均线","250日均线"] if not np.isnan(row[ma]) and row["收盘"] > row[ma]] , axis=1)
        # df["跌破均线"] = df.apply(lambda row: [ma for ma in ["5日均线","10日均线","20日均线","30日均线","60日均线","90日均线","250日均线"] if not np.isnan(row[ma]) and row["收盘"] < row[ma]] , axis=1)
        # df["突破布林带上轨"] = df.apply(lambda row: "突破上布林带" if row["收盘"] > row["BB_upper"] else "", axis=1)
        # df["跌破布林带下轨"] = df.apply(lambda row: "跌破下布林带" if row["收盘"] < row["BB_lower"] else "", axis=1)
        # 

        # 9.5 量价齐升 / 量价齐跌: count consecutive days, pct change, cum turnover
        df["量价齐升天数"] = 0
        df["量价齐升期间涨幅"] = "0.00%"
        df["量价齐升期间换手率"] = "0.00%"
        df["量价齐跌天数"] = 0
        ...
        df = df.drop(columns=[
            ...
            "pct_change", "MA5", "MA10", "MA20", "MA30", "MA60", "MA90", "MA250",
            "BB_mid", "BB_std", "BB_upper", "BB_lower"
        ], errors="ignore")
        

        indicators_list = df.tail(1).to_dict(orient="records")
        ...
        return {
            "status": "error",
            "error_message": f"An exception occurred during calculation: {str(e)}"
        }
