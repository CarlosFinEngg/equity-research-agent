...
2. Identify Key Categories, Trends & Insights (按维度划分，每个维度先列出数据指标，然后给出该维度的趋势分析，最后给出该维度的洞察):
...
* Profitability (盈利能力):
  ...
  • 趋势分析:
    ...
    - 特别关注指标的拐点，如ROE连续下降或毛利率突然下滑。

  • 盈利能力洞察 (Profitability Insights):
    * Insight 1: 并行分析近三年Diluted EPS和Weighted EPS的变化，判断公司盈利稳定性。
    * Insight 2: 根据净利润率与毛利率的差异，评估成本控制能力和盈利质量。
    * Insight 3: 若ROE或ROA出现连续两年以上下滑，提示可能存在结构性问题或盈利能力减弱。
...
* Growth Rates (增长率):
  ...
  • 数据指标:
    ...
    - Net Asset Growth (净资产增长率)
    - Total Asset Growth (总资产增长率)
  ...
  • 增长洞察 (Growth Insights):
    * Insight 1: 如果Revenue Growth高于Net Profit Growth，说明公司可能面临成本上升或利润率压缩的风险。
    * Insight 2: Net Asset Growth与Total Asset Growth的对比，可以揭示资本有效利用程度，若Total Asset Growth显著高于Net Asset Growth，说明负债扩张较快。
    * Insight 3: 当某一增长率从正转负，需要关注该增长背后是否只是季节性波动或是真正的增长停滞。

* Liquidity & Solvency (流动性与偿债能力):

  • 数据指标:
    - Current Ratio (流动比率)
    - Quick Ratio (速动比率)
    - Cash Ratio (现金比率)
    - Debt-to-Equity (负债与所有者权益比率)
    - Long-term Debt to Operating Funds (长期债务与营运资金比率)
    - Interest Coverage Ratio (利息支付倍数)
  ...
  • 流动性与偿债能力洞察 (Liquidity & Solvency Insights):
    * Insight 1: 如果流动比率持续低于1，说明短期流动性压力较大，可能需要关注现金流紧张风险。
    * Insight 2: Debt-to-Equity不断攀升，结合Interest Coverage Ratio下降，提示公司融资成本上升，偿债压力加强。
    * Insight 3: 若长期债务与营运资金比率的上升幅度远大于行业平均，需警惕债务扩张带来的潜在风险。

* Efficiency & Turnover (效率与周转):

  • 数据指标:
    - Total Asset Turnover (总资产周转率)
    - Inventory Turnover Days (存货周转天数)
  ...
  • 效率与周转洞察 (Efficiency & Turnover Insights):
    * Insight 1: 总资产周转率与净利润率联合分析，可以揭示盈利对资产利用效率的依赖程度。
    * Insight 2: 若存货周转天数显著增加，结合存货占用资金加大，可能导致资金成本上升，需要关注运营效率问题。
    * Insight 3: 应收账款周转率下降时，需评估应收账款质量和坏账风险。

* Cash Flow Metrics (现金流指标):

  • 数据指标:
    - Operating Cash Flow to Sales (经营现金净流量对销售收入比率)
    - Operating Cash Flow to Net Profit (经营现金净流量与净利润的比率)
    - Cash Flow Return on Assets (资产的经营现金流量回报率)
  ...
  • 现金流洞察 (Cash Flow Insights):
    * Insight 1: 若经营现金净流量对销售收入比率持续低于同行，可能存在利润质量较差的问题，需要关注利润的现金转化能力。
    * Insight 2: 经营现金流与净利润比率若长期小于1，意味着主要利润来自非现金项目或应收账款增加，需要警惕潜在的现金流风险。
    * Insight 3: 资产的经营现金流回报率若出现明显下滑，说明公司在资产使用上现金效率下降，需要关注资产负载和资本投入回报。

* Investment & Dividend Policies (投资与分红政策):

  • 数据指标:
    - Dividend Payout Ratio (股息发放率)
    - Investment Return Ratios (投资收益率)
    - Short-term vs. Long-term Investments Mix (短期与长期投资占比)
  ...
  • 投资与分红洞察 (Investment & Dividend Insights):
    * Insight 1: 高股息发放率伴随现金流充沛，说明公司现金分配策略稳健；但若分红率过高，可能削弱再投资能力。
    * Insight 2: 投资收益率持续下滑时，需关注管理层投资决策效率，是否存在踩雷项目。
    * Insight 3: 长期投资占比过高但回报率偏低，说明资金在非主营业务上压营运资金，需评估机会成本。

* Capital Structure & Leverage (资本结构与杠杆):

  • 数据指标:
    - Equity Ratio (股东权益比率)
    - Long-term Debt Ratio (长期负债比率)
    - Capitalization Ratio (资本化比率)
  ...
  • 资本结构与杠杆洞察 (Capital Structure & Leverage Insights):
    * Insight 1: 若资产负债率持续高于行业平均水平，提示公司必须关注偿债风险及再融资成本。
    * Insight 2: 股东权益比率下降且长期负债比率上升，结合利息支付倍数下降，说明杠杆水平过高，对盈利波动更敏感。
    * Insight 3: 资本化比率和负债结构的变化，需要与利率环境和公司现金流预测结合，评估未来偿债能力。

* Profit Composition (利润构成):

  • 数据指标:
    - Main Business Profit Ratio (主营业务利润率)
    - Non-Main Business Ratio (非主营比重)
    - Main Business Profit Proportion (主营利润比重)
  ...
  • 利润构成洞察 (Profit Composition Insights):
    * Insight 1: 当主营业务利润率高且稳定，非主营比重低，说明核心业务健康；反之则需警惕一次性收入或公允价值变动对业绩的影响。
    * Insight 2: 主营利润比重下降时，需审视非主营收益是否具有可持续性，否则可能造成未来利润波动。

3. DuPont Analysis (基于最新可用指标):
